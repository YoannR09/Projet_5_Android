...
public class MainActivity extends AppCompatActivity implements TasksAdapter.DeleteTaskListener {
    ...
    @Override
    public void onDeleteTask(TaskViewModel task) {
        mainActivityViewModel.deleteTask(task.getId());
        updateTasks();
    }
    ...
    private void updateTasks() {
        if (adapter.getCurrentData().size() == 0) {
            lblNoTasks.setVisibility(View.VISIBLE);
            listTasks.setVisibility(View.GONE);
        } else {
            lblNoTasks.setVisibility(View.GONE);
            ...
            switch (sortMethod) {
                case ALPHABETICAL:
                    Collections.sort(adapter.getCurrentData(), new TaskViewModel.TaskAZComparator());
                    break;
                case ALPHABETICAL_INVERTED:
                    Collections.sort(adapter.getCurrentData(), new TaskViewModel.TaskZAComparator());
                    break;
                case RECENT_FIRST:
                    Collections.sort(adapter.getCurrentData(), new TaskViewModel.TaskRecentComparator());
                    break;
                case OLD_FIRST:
                    Collections.sort(adapter.getCurrentData(), new TaskViewModel.TaskOldComparator());
                    break;

            }
            adapter.updateTasks(adapter.getCurrentData());
        }
    }
